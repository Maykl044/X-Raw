...
if not exist "%~dp0x86\X-Rav.exe" (
    echo [ОШИБКА] Не найден файл "%~dp0x86\X-Rav.exe"
    echo Возможно, Defender удалил .exe или папку x86. Сделай так:
...
powershell -NoProfile -Command "try { Get-ChildItem -Path '%~dp0x86' -Recurse -File | Unblock-File -ErrorAction SilentlyContinue } catch {}"

echo Запускаю X-Rav (x86)...
echo.
"%~dp0x86\X-Rav.exe"
