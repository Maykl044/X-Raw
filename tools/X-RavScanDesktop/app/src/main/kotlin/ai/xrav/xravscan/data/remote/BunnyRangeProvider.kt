 ...
 *
 *  1. Tries the direct edge-server JSON ([BunnyEdgeService]).
 ...
 *
 * Mirrors the Android `BunnyRangeProvider` byte-for-byte so the two
 * targets stay aligned. Never returns an empty list.
 */
class BunnyRangeProvider(
    private val service: BunnyEdgeService = BunnyEdgeService(),
) {
    private val log = LoggerFactory.getLogger("XRavScan.BunnyRangeProvider")
    ...
    suspend fun fetchAll(): Result =
        runCatching {
            val ips = service.edgeServers()
            if (ips.isEmpty()) error("Bunny API returned empty list")
            val grouped = groupToSubnets(ips)
            log.info("Bunny direct API: {} IPs → {} CIDR(s)", ips.size, grouped.size)
            Result(
                cidrs = grouped,
            ...
            log.warn("Bunny direct API failed — using hardcoded fallback: {}", t.message)
            Result(
                cidrs = HARDCODED_RANGES,
                source = Source.HARDCODED_FALLBACK,
                edgeIpCount = 0,
     ...
     * possible, and keep the original `/32` otherwise. Keeps the
     * discovery table compact (≤ 80 entries instead of 600+) while
